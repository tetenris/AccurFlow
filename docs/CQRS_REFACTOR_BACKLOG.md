...
2. [x] **`IMenuService`/`MenuService` — migrasi ke CQRS.** Dipakai sidebar via `@inject` di `Views/Shared/_Sidebar.cshtml` & `_Sidebar_Simple.cshtml` (`GetMenuHierarchyAsync` / `GetMenuHierarchyByRoleAsync`). Rencana: `Application/Features/Menus` → `GetMenuHierarchyQuery` (bisa filter role), view injeksi `ISender`; hapus `MenuService.cs` + `IMenuService.cs` + DI. Selesai 2026-08-19. `GetMenuHierarchyQuery` (record, `Guid? RoleId`) + `GetMenuHierarchyQueryHandler` (port 1:1, pakai `IRepository<MenuEntity/RoleEntity/RoleMenuEntity>`: role Superadmin/Admin → full menu, selain itu filter `CanView` + sertakan parent). Kedua sidebar injeksi `MediatR.ISender`. `Services/MenuService.cs` & `Services/Interfaces/IMenuService.cs` **DIHAPUS** + registrasi DI (`AppServiceCollection.cs`) dihapus. Build 0 error.
3. [x] **`IDocumentAttachmentService`/`DocumentAttachmentService` — migrasi ke CQRS.** Dipakai `DocumentAttachmentController` (`Datatable`, `GetByDocument`, `Upload` [IFormFile + config `Storage:*`], `Download`, `Delete`). Rencana: `Application/Features/DocumentAttachments` → `GetAttachmentDatatableQuery`, `GetAttachmentsByDocumentQuery`, `UploadAttachmentCommand`, `GetAttachmentFileQuery`, `DeleteAttachmentCommand`; controller → `ISender`; hapus `InventoryAndWorkflowServices.cs` + DI. Selesai 2026-08-19. Semua di `Application/Features/DocumentAttachments` (2 query + 3 command/handler, port 1:1). `UploadAttachmentCommandHandler` menerima `IFormFile` + baca config `Storage:*` (MaxFileSizeMb/AllowedExtensions/UploadsPath) + `IWebHostEnvironment`; `DeleteAttachmentCommandHandler` hapus fisik file + soft delete. `DocumentAttachmentController` → `ISender`. `Services/InventoryAndWorkflowServices.cs` (kini hanya berisi interface+class DocumentAttachment) **DIHAPUS** + registrasi DI dihapus. Build 0 error.

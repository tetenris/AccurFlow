...
- Receivable & Payable: daftar detail piutang/utang per customer/supplier. Menu `Accounting > Receivable & Payable` (pilih AR/AP + as of date) menampilkan tiap invoice terbuka (TotalAmount > PaidAmount, bukan Cancelled) dengan outstanding, status Open/Overdue (DueDate < as of date). Tanpa migrasi; menu parent Accounting scale seq 9.
- Pajak & PPN: CRUD pajak + laporan PPN sederhana. Menu `Accounting > Taxes` (tab Tax List CRUD rate pajak; tab PPN Report menampilkan invoice Posted bertax per periode: DPP = TotalAmount − TaxAmount, ringkasan PPN Keluaran (Sales) / PPN Masukan (Purchase) / net). Tanpa migrasi; menu parent Accounting seq 10 (Taxes).
- Aktiva Tetap: daftar aset + penyusutan garis lurus. Entity `FixedAssetEntity` & `FixedAssetDepreciationEntity`; menu `Accounting > Fixed Assets` (CRUD, kolom Computed MonthlyDepreciation & NetBookValue, aksi Depreciate per periode langsung posting jurnal Debit Depreciation Expense / Credit Accumulated Depreciation via JournalEntryService lalu simpan histori). Seed tambah COA `1-10500 Fixed Assets`, `1-10600 Accumulated Depreciation` (akun 5500 sudah ada di DB, seed skipped duplikat), dan `5-10500 Depreciation Expense`. Migrasi `20260817000433_AddFixedAssetModule`; menu parent Accounting seq 11.
- Tutup Tahun: proses penutupan tahun fiskal. Entity `YearEndClosingEntity`; menu `Accounting > Year-End Closing` (pilih tahun → preview saldo akun P&L Revenue/Other Income/Expense/Other Expense per `GetAccountBalanceAsync` as-of 31-12 tahun tersebut (total revenue, total expense, net income) → tombol Close membuat jurnal otomatis via JournalEntryService: Debit akun pendapatan & Credit akun beban untuk men-nolkan, balancing ke Retained Earnings (`30000000-...003`), lalu diposting dan dicatat `YearEndClosings` dengan unique index FiscalYear). Riwayat penutupan tampil di panel kanan. Migrasi `20260817001308_AddYearEndClosingModule`; menu parent Accounting seq 12.
- Jurnal Memo / Penyesuaian: jurnal manual non-operative. Kolom `JournalEntryEntity.JournalType` (General/Adjustment/Memo) dengan default `General`; nomor jurnal mengikuti tipe (`JE-`/`JA-`/`JM-`). Menu `Accounting > Jurnal Memo / Penyesuaian` (filter type/status, create modal dengan pilihan tipe Memo/Adjustment, dynamic lines + balance indicator, draft/edit/post/reverse). Memakai service `JournalEntryService` yang sama. Migrasi `20260817002321_AddJournalTypeToJournalEntry`; menu parent Accounting seq 13.
- Serial Number / Batch: pelacakan stok per unit/batch. Entity `StockBatchEntity`; menu `Inventory > Serial Number / Batch` (register batch/lot/serial per item: item, no batch, qty, expiry, notes; list dengan filter item + status Available/Partial/Out berdasarkan sisa; aksi Consume mengurangi RemainingQuantity dengan validasi ≤ sisa; delete hanya jika belum dipakai). Migrasi `20260817003003_AddSerialBatchModule`; menu parent Inventory seq 8 (Items 1, Stock Card 2, Stock Opname 3, Item Groups 4, Units 5, Stock Transfer 6, Stock Minimum 7, Serial Number / Batch 8).
- Payroll / HRM: master karyawan + penggajian. Entity `EmployeeEntity`, `PayrollEntity`, `PayrollLineEntity`; seed COA baru `2-10300 Payroll Payable` (GUID `20000000-...020` — GUID `...004` sudah terpakai legacy `2500 Long-term Debt`, seed akan skip duplikat). Menu `HRM / Payroll > Payroll` (tab Data Karyawan: CRUD employee dengan basic salary; tab Penggajian: create draft per periode dari semua karyawan aktif, auto total net salary, post → jurnal otomatis Debit `5200 Salaries and Wages` / Credit `Payroll Payable`, nomor `PR-YYYYMM-XXXX`). Migrasi `20260817003809_AddPayrollModule`; menu parent HRM/Payroll baru seq 11 (top-level), child Payroll seq 1.
- Produksi / Manufacturing: BOM + production order. Entity `BillOfMaterialEntity`+`BillOfMaterialLineEntity`, `ProductionOrderEntity`+`ProductionOrderLineEntity`. Menu `Produksi > Production` (tab Bill of Material: pilih finished item + komponen dengan qty per unit; tab Production Order: pilih BOM + warehouse + qty produksi → draft berisi komponen = qty BOM × qty produksi, post → cek stok komponen, buat `StockMovement` "Production Consumption" (out per komponen) & "Production Output" (in barang jadi), jurnal otomatis Debit Inventory barang jadi / Credit Inventory komponen (biaya = PurchasePrice komponen), nomor `PRD-XXXXX`, BOM `BOM-XXXXX`). Migrasi `20260817005129_AddProductionModule`; menu parent Produksi baru seq 12 (top-level), child Production seq 1. **Backlog menu Accurate selesai (41 menu terhubung).**
